...
class _DashboardPageState extends State<DashboardPage> {
  // convenience getters to read current app data
  List<ReminderModel> get _allReminders => AppData.instance.reminders;
  List<ChecklistModel> get _allChecklists => AppData.instance.checklists;

  // active reminders: active == true AND checklist exists AND checklist is not fully checked
  List<ReminderModel> get _activeReminders {
    return _allReminders.where((r) {
      if (!r.active) return false;
      if (r.checklistTitle == null) return false;
      final checklist = _allChecklists.firstWhere(
        (c) => c.title == r.checklistTitle,
        orElse: () => ChecklistModel(title: '__MISSING__', category: '__MISSING__'),
      );
      if (checklist.title == '__MISSING__') return false;
      if (checklist.items.isEmpty) return false;
      final completed = checklist.items.where((it) => it.checked).length;
  ...
  void initState() {
    ...
    // Check for alerts
    NotificationService().checkRemindersAndGenerateAlerts();
  }

  void _openChecklistForReminder(ReminderModel reminder) {
    if (reminder.checklistTitle == null) return;
    final checklistIndex = _allChecklists.indexWhere((c) => c.title == reminder.checklistTitle);
        ...
        return StatefulBuilder(builder: (modalContext, setModalState) {
          return DraggableScrollableSheet(
            ...
            builder: (_, controller) {
              final checklist = AppData.instance.checklists[checklistIndex];
  ...
  Widget build(BuildContext context) {
    ...
    final todaysRemindersCount = _allReminders.where((r) {
      if (r.date == null) return false;
      final d = r.date!;
      return d.year == today.year && d.month == today.month && d.day == today.day;
    }).length;

    final activeChecklistCount = AppData.instance.checklists.where((c) {
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  _buildActiveRemindersCard(context, activeReminders, theme),
                  const SizedBox(height: 16),
      ...
      child: Row(
        children: [
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: const [
              Text('Total alerts', style: TextStyle(color: Colors.white70)),
              SizedBox(height: 6),
              Text('0', style: TextStyle(color: Colors.white, fontSize: 26, fontWeight: FontWeight.w800)),
            ],
          ),
          const Spacer(),
          ElevatedButton(
  ...
  Widget _buildActiveRemindersCard(BuildContext context, List<ReminderModel> activeReminders, AppTheme theme) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (activeReminders.isEmpty)
            ...
            ...List.generate(activeReminders.length, (i) {
              final r = activeReminders[i];
              final checklist = _allChecklists.firstWhere((c) => c.title == r.checklistTitle, orElse: () => ChecklistModel(title: '__MISSING__', category: '__MISSING__'));
              return Padding(
                    ...
                    children: [
                      Expanded(
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          const SizedBox(height: 6),
                          Text(checklist.title == '__MISSING__' ? 'Checklist not found' : checklist.title, style: const TextStyle(color: Colors.black54, fontSize: 12)),
                          const SizedBox(height: 6),
                          Row(children: [
                            const Icon(Icons.calendar_today, size: 13, color: Colors.black45),
                            const SizedBox(width: 6),
                            Text(r.date == null ? '--' : '${r.date!.month}/${r.date!.day}/${r.date!.year}', style: const TextStyle(fontSize: 12, color: Colors.black54)),
                            const SizedBox(width: 10),
                            const Icon(Icons.access_time, size: 13, color: Colors.black45),
                            const SizedBox(width: 6),
                            Text(r.time == null ? '--' : r.time!.format(context), style: const TextStyle(fontSize: 12, color: Colors.black54)),
                          ]),
                        ]),
                      ),
                      ElevatedButton(
